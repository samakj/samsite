...
): StateObjectType<GitCommitStateType> => response.reduce(
    (acc: StateObjectType<GitCommitStateType>, commit: GitRepoCommitResponseType): StateObjectType<GitCommitStateType> => {
        acc[commit.sha] = {
            url: commit.html_url,
            sha: commit.sha,
