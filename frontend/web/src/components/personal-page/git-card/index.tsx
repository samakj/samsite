...
import {
    GitCardPropsType,
    GitCommitRepoPropsType,
    GitCommitTitlePropsType,
    GitCommitDescriptionPropsType,
...
const GitCommitRepo: React.FunctionComponent<GitCommitRepoPropsType> = ({ commit }) =>
    commit ? (
        <div className="commit-repo">{commit.url.split('/')[4]}</div>
    ) : (
...
const DumbGitCard: React.FunctionComponent<GitCardPropsType> = ({
    ...
    useEffect(
        () => {
            repos &&
                Object.values(repos).forEach((repo: GitRepoStateType): void =>
                    onFetchGitRepoCommits(username, repo.name),
                );
        },
