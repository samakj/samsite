...
): GitUserStateType => username && fetchGitUserStoreHandler.storeMaps.getKeyValue(state, username);
...
export const getAllGitUsersSelector = (
    state: StoreObjectType<GitUserStateType>,
): StateObjectType<GitUserStateType> => fetchGitUserStoreHandler.storeMaps.getValue(state);
